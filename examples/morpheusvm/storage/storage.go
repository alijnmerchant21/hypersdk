...
// TotalSupplyKey returns the key used to store the total supply of the token
func TotalSupplyKey() []byte {
	return []byte(TotalSupplyKeyPrefix)
}

// GetTotalSupply retrieves the total supply from the state
func GetTotalSupply(ctx context.Context, r state.Immutable) (uint64, error) {
	key := TotalSupplyKey()
	...
	if val == nil {
		return 0, nil // Return 0 if the key doesn't exist
	}
	return decodeUint64(val)
}

// SetTotalSupply sets the total supply in the state
func SetTotalSupply(ctx context.Context, mu state.Mutable, supply uint64) error {
	key := TotalSupplyKey()
